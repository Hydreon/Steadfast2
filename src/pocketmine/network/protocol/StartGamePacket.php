...
//		['name' => 'showcoordinates', 'type' => 1, 'value' => 1]
	...
	public function encode($playerProtocol){
		...
		if($playerProtocol >= Info::PROTOCOL_418){
			$this->putVarInt(0); // Experiments Array size
			// Experiement Element: ToggleName (string), Enabled (bool)
			$this->putByte(0); // Where experiments ever Toggled
		}
		...
		if($playerProtocol >= Info::PROTOCOL_418){
			$this->putByte(0); // Start with map enabled
		}
		...
		if ($playerProtocol >= Info::PROTOCOL_407) {
			$this->putByte(0); //exp gameplay override
		}
