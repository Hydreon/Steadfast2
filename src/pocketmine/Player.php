...
//		$pk->isEncoded = true;
     ...
     * @return Player[]
     */
    public function getSubClients() : array{
...
//						$this->addEffect(Effect::getEffect(Effect::ABSORPTION)->setAmplifier(0)->setDuration(120 * 20));
    ...
    public function handleAdventureSettings(AdventureSettingsPacket $packet) : bool{
        ...
        $isHacker = ($this->allowFlight === false && $isFlying) || (!$this->isSpectator() && ($packet->flags >> 7) & 0x01 === 1);
        if($isHacker){
    ...
    public function handleMovePlayer(MovePlayerPacket $packet) : bool{
        if(!$this->dead && $this->spawned){
            ...
            if($this->isTeleporting && $newPos->distanceSquared($this) > 2){
                ...
                $sqrtPos = $this->newPosition !== null ? $this->newPosition : $this;
                $distanceSquared = ($newPos->x - $sqrtPos->x) ** 2 + ($newPos->z - $sqrtPos->z) ** 2;
    ...
    public function handleUseItem(UseItemPacket $packet) : bool{
        if(!$this->spawned or $this->dead or $this->blocked){
            return true;
        }
    ...
    public function handleText(TextPacket $packet) : bool{
        ...
        if($packet->type === TextPacket::TYPE_CHAT){
            ...
            $this->server->getLogger()->debug("Recive message with type ".$packet->type);
        }
    ...
    public function handleTileEntityData(TileEntityDataPacket $packet) : bool{
        if(!$this->spawned or $this->blocked or $this->dead){
            return true;
        }
    ...
    public function handleModalFormResponse(ModalFormResponsePacket $packet) : bool{
        $this->checkModal($packet->formId, json_encode($packet->data, true));
        return true;
    }

    /**
     * @minProtocol 120
     * @param PurchaseReceiptPacket $packet
     * @return bool
     */
    ...
    public function handleDisconnect(DisconnectPacket $packet){
    ...
    public function handleDataPacket(DataPacket $packet) : bool{
        ...
        if(!$this->isOnline() && !$packet->isAvailableBeforeLogin()){
        ...
        /**
         * A Basic Handler Based From Turanic
         */
        $handleName = "handle" . str_ireplace("Packet", "", $packet->getName());

        $handled = false;
        if(is_callable([$this, $handleName])){
            if($this->{$handleName}($packet)){
                $handled = true;
            }
        }

        if(!$handled){
            $this->server->getLogger()->debug("Unhandled " . $packet->getName() . " received from " . $this->username . ": 0x" . bin2hex($packet->buffer));
            return false;
        }
...
//		$this->loggedIn = true;
    ...
    protected function addTransaction(BaseTransaction $transaction){
        ...
        if($this->currentWindow === null || $this->currentWindow === $transaction->getInventory()) {
    ...
    public function updateSpeed(float $value){
    ...
    public function isHaveElytra() {
        return $this->getInventory()->getArmorItem(Elytra::SLOT_NUMBER) instanceof Elytra;
    }
    ...
    protected function useItem(Item $item, int $slot, int $face, Vector3 $blockPosition, Vector3 $clickPosition) {
        switch($face){
            ...
            case 5:
                ...
                if($blockPosition->distance($this) > 10 || ($this->isCreative() && $this->isAdventure())) {
            ...
            case -1:  // -1 for 0.16
                ...
                $vector0 = new Vector3(0, 0, 0);
                if(!$blockPosition->equals($vector0)) {
                    $vectorLength = $blockPosition->distance($vector0);
                    $aimPos = $blockPosition->divide($vectorLength);
                }else{
                    $aimPos = $vector0;
                }
    ...
    private function breakBlock(Vector3 $blockPosition) {
        ...
        if($this->level->useBreakOn($blockPosition, $item, $this) === true){
            if($this->isSurvival()){
    ...
    private function normalTransactionLogic(InventoryTransactionPacket $packet) {
        ...
        /** @var SimpleTransactionData $trData */
     ...
     * @throws \Exception
     */
    private static function tryApplyCraft(&$craftSlots, Recipe $recipe) {
        if ($recipe instanceof ShapedRecipe) {
            $ingredients = [];
...
//					echo $item . PHP_EOL;
     ...
     * @return bool
     */
    public function subAuth(SubClientLoginPacket $packet, Player $parent) : bool{
        $this->username = TextFormat::clean($packet->username);
        $this->xblName = $this->username;
        $this->displayName = $this->username;
        $this->setNameTag($this->username);
